...
for (i in unique(NEI$year)){
  year_data <- subset(NEI,NEI$year == i & NEI$fips == "24510")
  year <- c(year,i)
...
title(ylab = "Total PM2.5 emission(tonnes)",line = 5)

title(xlab = "Year",line = 2.5)
title(main="PM2.5 Emissions in each year at Baltimore City, Maryland")
dev.off()
